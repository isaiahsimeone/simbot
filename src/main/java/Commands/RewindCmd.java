...
public class RewindCmd {
    ...
    public static boolean execute(SimPlayer simplayer) {
        ...
        if (current_position - 5000 <= 0)
            return false;//set_pos = playing_track.getDuration() - 1; // skip to the next song
        else
            set_pos = current_position - 5000; // rewind 5 seconds
